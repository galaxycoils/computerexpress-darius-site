...
function defineComponent(config) {
	return (0, _openuidev_lang_core.defineComponent)(config);
}
function createLibrary(input) {
	return (0, _openuidev_lang_core.createLibrary)(input);
}
//#endregion
//#region src/context.ts
const OpenUIContext = (0, react.createContext)(null);
/**
* Access the full OpenUI context. Throws if used outside a <Renderer />.
*/
function useOpenUI() {
	const ctx = (0, react.useContext)(OpenUIContext);
...
function useOpenUIState({ response, library, isStreaming, onAction, onStateUpdate, initialState, toolProvider, onError }, renderDeep) {
	const sp = (0, react.useMemo)(() => (0, _openuidev_lang_core.createStreamingParser)(library.toJSONSchema(), library.root), [library]);
	...
	const store = (0, react.useMemo)(() => (0, _openuidev_lang_core.createStore)(), []);
	const queryManager = (0, react.useMemo)(() => (0, _openuidev_lang_core.createQueryManager)(toolProvider ?? null), [toolProvider]);
	(0, react.useEffect)(() => {
	...
	(0, react.useEffect)(() => {
		...
		const evaluatedNodes = (result?.queryStatements ?? []).map((qn) => {
			...
			return {
				...
				toolName: qn.toolAST ? (0, _openuidev_lang_core.evaluate)(qn.toolAST, evaluationContext) : "",
				args: qn.argsAST ? (0, _openuidev_lang_core.evaluate)(qn.argsAST, evaluationContext) : null,
				defaults: qn.defaultsAST ? (0, _openuidev_lang_core.evaluate)(qn.defaultsAST, evaluationContext) : null,
				refreshInterval: qn.refreshAST ? (0, _openuidev_lang_core.evaluate)(qn.refreshAST, evaluationContext) : void 0,
	...
	(0, react.useEffect)(() => {
		...
		const nodes = (result?.mutationStatements ?? []).map((mn) => ({
			...
			toolName: mn.toolAST ? (0, _openuidev_lang_core.evaluate)(mn.toolAST, evaluationContext) : ""
		}));
	...
	const triggerAction = (0, react.useCallback)(async (userMessage, formName, action) => {
		...
		if (action && !("steps" in action)) {
			const actionType = action.type || _openuidev_lang_core.BuiltinActionType.ContinueConversation;
		...
		if (actionPlan?.steps) {
			for (const step of actionPlan.steps) switch (step.type) {
				case _openuidev_lang_core.ACTION_STEPS.Run:
					if (step.refType === "mutation") {
						const mn = resultRef.current?.mutationStatements?.find((m) => m.statementId === step.statementId);
						const evaluatedArgs = mn?.argsAST ? (0, _openuidev_lang_core.evaluate)(mn.argsAST, evaluationContext) : {};
						if (!await queryManager.fireMutation(step.statementId, evaluatedArgs)) return;
					} else queryManager.invalidate([step.statementId]);
					break;
				case _openuidev_lang_core.ACTION_STEPS.ToAssistant:
					handler?.({
						type: _openuidev_lang_core.BuiltinActionType.ContinueConversation,
					...
					break;
				case _openuidev_lang_core.ACTION_STEPS.OpenUrl:
					handler?.({
						type: _openuidev_lang_core.BuiltinActionType.OpenUrl,
						params: { url: step.url },
					...
					break;
				case _openuidev_lang_core.ACTION_STEPS.Set: {
					if (!step.valueAST) {
						console.warn(`[openui] Set action for ${step.target} has no valueAST — skipping`);
						break;
					}
					const value = (0, _openuidev_lang_core.evaluate)(step.valueAST, evaluationContext);
					store.set(step.target, value);
					break;
				}
				case _openuidev_lang_core.ACTION_STEPS.Reset: {
	...
	const evaluatedResult = (0, react.useMemo)(() => {
		...
		try {
			const evaluatedRoot = (0, _openuidev_lang_core.evaluateElementProps)(result.root, evalCtx);
	...
	(0, react.useEffect)(() => {
		...
		if (result?.meta?.errors?.length) errors.push(...(0, _openuidev_lang_core.enrichErrors)(result.meta.errors, library.toJSONSchema(), Object.keys(library.components)));
...
function Renderer({ response, library, isStreaming = false, onAction, onStateUpdate, initialState, onParseResult, toolProvider, queryLoader, onError }) {
	...
	const stableToolProvider = (0, react.useRef)({ async callTool(toolName, args) {
		...
		if (typeof current.callTool === "function") return (0, _openuidev_lang_core.extractToolResult)(await current.callTool({
			name: toolName,
			arguments: args
		}));
		const map = current;
		const fn = map[toolName];
		if (!fn) throw new _openuidev_lang_core.ToolNotFoundError(toolName, Object.keys(map));
		return fn(args);
	} });
	...
	return /* @__PURE__ */ (0, react_jsx_runtime.jsx)(OpenUIContext.Provider, {
		value: contextValue,
...
function reactive(schema) {
	(0, _openuidev_lang_core.markReactive)(schema);
	return schema;
}
//#endregion
...
function useStateField(name, value) {
	...
	return (0, _openuidev_lang_core.resolveStateField)(name, value, ctx.store ?? null, ctx.evaluationContext ?? null, (fieldName) => ctx.getFieldValue(formName, fieldName), (fieldName, nextValue) => ctx.setFieldValue(formName, void 0, fieldName, nextValue));
}
//#endregion
...
function useCreateFormValidation() {
	...
	const validateField = (0, react.useCallback)((name, value, rules) => {
		const error = (0, _openuidev_lang_core.validate)(value, rules);
		setErrors((prev) => {
	...
	const validateForm = (0, react.useCallback)(() => {
		...
		for (const [name, reg] of Object.entries(fieldsRef.current)) {
			...
			const error = (0, _openuidev_lang_core.validate)(value, reg.rules);
...
Object.defineProperty(exports, "ACTION_STEPS", {
	enumerable: true,
	get: function() {
		return _openuidev_lang_core.ACTION_STEPS;
	}
});
Object.defineProperty(exports, "BuiltinActionType", {
	enumerable: true,
	get: function() {
		return _openuidev_lang_core.BuiltinActionType;
	}
});
...
Object.defineProperty(exports, "ToolNotFoundError", {
	enumerable: true,
	get: function() {
		return _openuidev_lang_core.ToolNotFoundError;
	}
});
Object.defineProperty(exports, "builtInValidators", {
	enumerable: true,
	get: function() {
		return _openuidev_lang_core.builtInValidators;
	}
});
...
Object.defineProperty(exports, "createParser", {
	enumerable: true,
	get: function() {
		return _openuidev_lang_core.createParser;
	}
});
Object.defineProperty(exports, "createStreamingParser", {
	enumerable: true,
	get: function() {
		return _openuidev_lang_core.createStreamingParser;
	}
});
...
Object.defineProperty(exports, "extractToolResult", {
	enumerable: true,
	get: function() {
		return _openuidev_lang_core.extractToolResult;
	}
});
Object.defineProperty(exports, "generatePrompt", {
	enumerable: true,
	get: function() {
		return _openuidev_lang_core.generatePrompt;
	}
});
Object.defineProperty(exports, "isReactiveAssign", {
	enumerable: true,
	get: function() {
		return _openuidev_lang_core.isReactiveAssign;
	}
});
Object.defineProperty(exports, "mergeStatements", {
	enumerable: true,
	get: function() {
		return _openuidev_lang_core.mergeStatements;
	}
});
Object.defineProperty(exports, "parseRules", {
	enumerable: true,
	get: function() {
		return _openuidev_lang_core.parseRules;
	}
});
Object.defineProperty(exports, "parseStructuredRules", {
	enumerable: true,
	get: function() {
		return _openuidev_lang_core.parseStructuredRules;
	}
});
...
Object.defineProperty(exports, "tagSchemaId", {
	enumerable: true,
	get: function() {
		return _openuidev_lang_core.tagSchemaId;
	}
});
exports.useCreateFormValidation = useCreateFormValidation;
exports.useFormName = useFormName;
exports.useFormValidation = useFormValidation;
exports.useGetFieldValue = useGetFieldValue;
exports.useIsStreaming = useIsStreaming;
exports.useRenderNode = useRenderNode;
exports.useSetDefaultValue = useSetDefaultValue;
exports.useSetFieldValue = useSetFieldValue;
exports.useStateField = useStateField;
exports.useTriggerAction = useTriggerAction;
Object.defineProperty(exports, "validate", {
	enumerable: true,
	get: function() {
		return _openuidev_lang_core.validate;
	}
});
