...
async function run() {
  ...
  if (cssFile) {
    ...
    const cssTag = `<link rel="stylesheet" crossorigin href="${cssHref}" />`
    if (!assetAlreadyInjected(template, cssHref)) {
      template = template.replace('</head>', `    ${cssTag}\n</head>`)
    }
  }
  if (jsFile) {
    const jsSrc = `/assets/${jsFile}`
    const jsTag = `<script type="module" crossorigin src="${jsSrc}"></script>`
    if (!assetAlreadyInjected(template, jsSrc)) {
      template = template.replace('</head>', `    ${jsTag}\n</head>`)
    }
  }
  if (vendorFile) {
    const vendorHref = `/assets/${vendorFile}`
    const vendorTag = `<link rel="modulepreload" crossorigin href="${vendorHref}" />`
    if (!assetAlreadyInjected(template, vendorHref)) {
      template = template.replace('</head>', `    ${vendorTag}\n</head>`)
    }
  }
